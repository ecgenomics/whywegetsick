...
# Check no difference between cases and controls)
...
```{r, eval=FALSE}
write.csv(df_plot, file = "demography_main_figure_62diseases_nchildren.tsv", row.names = FALSE)
```
...
p3 <- p3 + theme(
    ...
    axis.title.x = element_text(size = 7, family = "Helvetica", color = "black", margin = margin(t = 10)),
    axis.title.y = element_text(size = 7, family = "Helvetica", color = "black", margin = margin(r = 10)),
    legend.title = element_text(size = 7, family = "Helvetica", color = "black", face = "bold"),
...
```{r}
ggsave(
  filename = "Figure4_demography.pdf",
  plot = p3,
  device = "pdf",
  width = 180,
  height = 130,
