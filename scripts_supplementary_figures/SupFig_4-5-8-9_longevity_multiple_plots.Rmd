---
title: "Longevity Multiple Plots"
author: "Eva Brigos"
date: "2024-10-18"
...
```{r}
#setwd("~/Documents/plots_paper/results_paper")

# Genetic Correlations
gc <- read.csv("../data/longevity-genetic-correlations.txt", sep = "\t")

# Counts pleiotropic SNPs by disease
pleiotropy <- read.csv("../data/longevity-commonSNPs-pleioFDR-conjfdr005-pleiotropies.csv", sep = ",")

# Pleiotropy: all information by pleiotropic SNP
pleiotropy2 <- read.csv("../data/plifespan_clumped_pleiotropies_62Diseases_commonSNPs_conjfdr005.csv", sep = ",")
...
Neither the GC nor the ADO are normally distributed.
...
```{r}
ggsave("./longevity_gc_by_domain.png", gc_domains_plot, height = 170, width = 180, units = "mm", dpi = 400)
```
...
```{r}
ggsave2("./correlation_longevity_GC_ADO5_pvalue.png", corr_gc_ado, height = 100, width = 110, units = "mm", dpi = 400)
```

Do the plot with FDR significant:

```{r}
# Filter for significance
df$SignificantFDR <- ifelse(df$FDR < 0.05, "Significant", "Not Significant")
df3 <- df %>% filter(FDR < 0.05)
nrow(df3)
```
...
```{r}
correlation_result <- cor.test(df3$GC, df3$ADO_5, method = "spearman")
print(correlation_result)
correlation <- correlation_result$estimate
p_value <- correlation_result$p.value
...
```{r}
corr_gc_ado2 <- ggplot(df, aes(x = ADO_5, y = GC, color = Correlation)) +
  geom_point(alpha = 0.3, size = 2) +
  geom_point(data = df, aes(alpha = SignificantFDR), size = 2)+
  scale_color_manual(values = c("Positive GC" = "#023E8A", "Negative GC" = "#990000")) +
  geom_smooth(data = df3, method = "lm", se = TRUE, color = "black", size = 0.5) +
  theme_bw() +
  annotate("text", x = Inf, y = Inf,
           label = paste("R =", round(correlation, 2), "\nP =",
                         format(p_value, scientific = TRUE)), 
           hjust = 1.1, vjust = 2, size = 3, color = "black")
# remove legend and other modifications
corr_gc_ado2 <- corr_gc_ado2 + theme(legend.position = "none",
                     axis.title.x = element_text(size = 7, margin = margin(t = 20), face = "bold", family = "Arial"),
                     axis.title.y = element_text(size = 7, margin = margin(r = 20), face = "bold", family = "Arial"),
                     axis.text.y = element_text(size = 7, family = "Arial"),
                     axis.text.x = element_text(size = 7, family = "Arial"),
                     ...
                     panel.grid = element_blank())
# rename x axis label
corr_gc_ado2 <- corr_gc_ado2 + ylab("rg (LDSC)")
corr_gc_ado2 <- corr_gc_ado2 + xlab("Age at disease onset")
```

```{r}
ggsave2("./correlation_longevity_GC_ADO5_fdr.png", corr_gc_ado2, height = 100, width = 110, units = "mm", dpi = 400)
```
...
```{r}
ggsave("./longevity_gencorr_fdr.png",
       subplot1, 
       height = 170, 
       width = 100, 
       units = "mm",
